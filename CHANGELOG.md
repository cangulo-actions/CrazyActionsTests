# 57.0.0 test semver pr41 push 3 commits break (#345)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 56.0.2 fix: commit that fixes a bug (#344)

## patches:
* fix: commit that fixes a bug (#344)

# 56.0.1 fix: reset repo before test

## patches:
* fix: reset repo before test

# 56.0.0 test semver pr41 push 3 commits break (#342)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 55.0.2 fix: commit that fixes a bug (#341)

## patches:
* fix: commit that fixes a bug (#341)

# 55.0.1 fix: reset repo before test

## patches:
* fix: reset repo before test

# 55.0.0 test semver pr41 push 3 commits break (#339)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 54.0.4 test semver pr41 push 1 commit fix (#338)

## patches:
* fix: commit that fixes a bug
## others
* ci: updated [cd.yml] to use cangulo-actions/semver@#38
* ---------

# 54.0.3 fix: reset repo before test

## patches:
* fix: reset repo before test

# 54.0.2 fix: reset repo before test

## patches:
* fix: reset repo before test

# 54.0.1 fix: reset repo before test

## patches:
* fix: reset repo before test

# 54.0.0 test semver pr41 push 3 commits break (#337)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 53.0.3 fix: commit that fixes a bug (#336)

## patches:
* fix: commit that fixes a bug (#336)

# 53.0.2 fix: reset repo before test

## patches:
* fix: reset repo before test

# 53.0.1 fix: reset repo before test

## patches:
* fix: reset repo before test

# 53.0.0 test semver pr41 push 3 commits break (#335)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 52.0.2 fix: commit that fixes a bug (#334)

## patches:
* fix: commit that fixes a bug (#334)

# 52.0.1 fix: reset repo before test

## patches:
* fix: reset repo before test

# 52.0.0 test semver pr41 push 3 commits break (#333)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 51.0.2 test semver pr41 push 1 commit fix (#332)

## patches:
* fix: commit that fixes a bug
## others
* ci: updated [cd.yml] to use cangulo-actions/semver@#38
* ---------

# 51.0.1 fix: reset repo before test

## patches:
* fix: reset repo before test

# 51.0.0 test semver pr41 3 commit break (#330)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 50.0.16 test semver pr41 1 commit fix (#329)

## patches:
* fix: commit that fixes a bug
## others
* ci: updated [cd.yml] to use cangulo-actions/semver@#38
* ---------

# 50.0.15 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.14 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.13 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.12 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.11 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.10 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.9 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.8 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.7 create reset workflow (#327)

## patches:
* fix: created workflow for resetting the repo
* fix: rename workflow
## others
* ---------

# 50.0.6 fix: reset repo before test

## patches:
* fix: reset repo before test

# 50.0.5 fix: created workflow for resetting the repo (#326)

## patches:
* fix: created workflow for resetting the repo (#326)

# 50.0.4 fix: test semver PR 90 - reset release cycle before running cangulo-actions/semver tests (#325)

## patches:
* fix: test semver PR 90 - reset release cycle before running cangulo-actions/semver tests (#325)

# 50.0.3 fix: test semver PR 18 - reset release cycle before running cangulo-actions/semver tests (#324)

## patches:
* fix: test semver PR 18 - reset release cycle before running cangulo-actions/semver tests (#324)

# 50.0.2 fix: test semver PR 85 - reset release cycle before running cangulo-actions/semver tests (#323)

## patches:
* fix: test semver PR 85 - reset release cycle before running cangulo-actions/semver tests (#323)

# 50.0.1 fix: test semver PR 41 - reset release cycle before running cangulo-actions/semver tests

## patches:
* fix: test semver PR 41 - reset release cycle before running cangulo-actions/semver tests

# 50.0.0 test semver pr41 3 commit break (#321)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#38
## others
* ---------

# 49.0.2 fix: commit that fixes a bug (#320)

## patches:
* fix: commit that fixes a bug (#320)

# 49.0.1 fix(src,tfm): [attempt 7] created initial versions (#317)

## patches:
* fix(src,tfm): [attempt 7] created initial versions (#317)

# 49.0.0 test semver pr3 commit break (#316)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 48.0.3 fix: commit that fixes a bug (#315)

## patches:
* fix: commit that fixes a bug (#315)

# 48.0.2 fix(src,tfm): [attempt 6] created initial versions (#313)

## patches:
* fix(src,tfm): [attempt 6] created initial versions (#313)

# 48.0.1 fix(src,tfm): [attempt 5] created initial versions (#311)

## patches:
* fix(src,tfm): [attempt 5] created initial versions (#311)

# 48.0.0 test semver pr3 commit break (#310)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 47.0.1 fix: commit that fixes a bug (#309)

## patches:
* fix: commit that fixes a bug (#309)

# 47.0.0 test semver pr3 commit break (#307)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 46.0.2 fix: commit that fixes a bug (#306)

## patches:
* fix: commit that fixes a bug (#306)

# 46.0.1 fix(src,tfm): [attempt 4] created initial versions (#304)

## patches:
* fix(src,tfm): [attempt 4] created initial versions (#304)

# 46.0.0 test semver pr3 commit break (#303)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 45.0.1 fix: commit that fixes a bug (#302)

## patches:
* fix: commit that fixes a bug (#302)

# 45.0.0 test semver pr3 commit break (#300)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 44.0.1 fix: commit that fixes a bug (#299)

## patches:
* fix: commit that fixes a bug (#299)

# 44.0.0 test semver pr3 commit break (#297)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 43.0.1 fix: commit that fixes a bug (#296)

## patches:
* fix: commit that fixes a bug (#296)

# 43.0.0 test semver pr3 commit break (#294)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 42.0.1 fix: commit that fixes a bug (#293)

## patches:
* fix: commit that fixes a bug (#293)

# 42.0.0 test semver pr3 commit break (#291)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 41.0.3 fix: commit that fixes a bug (#290)

## patches:
* fix: commit that fixes a bug (#290)

# 41.0.2 fix(src,tfm): [attempt 3] created initial versions (#288)

## patches:
* fix(src,tfm): [attempt 3] created initial versions (#288)

# 41.0.1 fix(src,tfm): [attempt 2] created initial versions (#287)

## patches:
* fix(src,tfm): [attempt 2] created initial versions (#287)

# 41.0.0 test semver pr3 commit break (#285)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 40.0.1 fix: commit that fixes a bug (#284)

## patches:
* fix: commit that fixes a bug (#284)

# 40.0.0 test semver pr3 commit break (#282)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 39.0.2 fix: commit that fixes a bug (#281)

## patches:
* fix: commit that fixes a bug (#281)

# 39.0.1 fix: updated cd to print semver output in the GH summary and updated readme

## patches:
* fix: updated cd to print semver output in the GH summary and updated readme

# 39.0.0 test semver pr3 commit break (#279)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#32
## others
* ---------

# 38.0.1 fix: commit that fixes a bug (#278)

## patches:
* fix: commit that fixes a bug (#278)

# 38.0.0 test semver pr3 commit break (#275)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@#35
## others
* ---------

# 37.0.2 fix: commit that fixes a bug (#274)

## patches:
* fix: commit that fixes a bug (#274)

# 37.0.1 fix: Added src/lambda1 and terraform/main.tf

## patches:
* fix: Added src/lambda1 and terraform/main.tf

# 37.0.0 test semver pr3 commit break (#271)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@test-gh-action
## others
* ---------

# 36.0.1 fix: commit that fixes a bug (#270)

## patches:
* fix: commit that fixes a bug (#270)

# 36.0.0 test semver pr3 commit break (#267)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-title
## others
* ---------

# 35.0.3 fix: commit that fixes a bug (#266)

## patches:
* fix: commit that fixes a bug (#266)

# 35.0.2 fix: GH rename step

## patches:
* fix: GH rename step

# 35.0.1 fix: renamed CI

## patches:
* fix: renamed CI

# 35.0.0 test semver pr3 commit break (#212)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@test-gh-action
## others
* ---------

# 34.0.2 fix: commit that fixes a bug (#211)

## patches:
* fix: commit that fixes a bug (#211)

# 34.0.1 fix: added ci.yml for testing cangulo-actions/conventional-commits-validator

## patches:
* fix: added ci.yml for testing cangulo-actions/conventional-commits-validator

# 34.0.0 test semver pr3 commit break (#207)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-on-pr-close
## others
* ---------

# 33.0.1 fix: commit that fixes a bug (#206)

## patches:
* fix: commit that fixes a bug (#206)

# 33.0.0 test semver pr3 commit break (#203)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@main
## others
* ---------

# 32.0.1 fix: commit that fixes a bug (#202)

## patches:
* fix: commit that fixes a bug (#202)

# 32.0.0 test semver pr3 commit break (#199)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@refactor
## others
* ---------

# 31.0.1 fix: commit that fixes a bug (#198)

## patches:
* fix: commit that fixes a bug (#198)

# 31.0.0 test semver pr3 commit break (#189)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@HEAD
## others
* ---------

# 30.0.1 fix: commit that fixes a bug (#188)

## patches:
* fix: commit that fixes a bug (#188)

# 30.0.0 test semver pr3 commit (#181)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean
## others
* ---------

# 29.0.1 fix: [cd.yml] updated to cangulo-actions/semver@clean (#179)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean (#179)

# 29.0.0 test semver pr3 commit (#178)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean
## others
* ---------

# 28.0.1 fix: [cd.yml] updated to cangulo-actions/semver@clean (#176)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean (#176)

# 28.0.0 test semver pr3 commit (#175)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean
## others
* ---------

# 27.0.1 fix: [cd.yml] updated to cangulo-actions/semver@clean (#173)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean (#173)

# 27.0.0 test semver pr3 commit (#171)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean
## others
* ---------

# 26.0.1 fix: [cd.yml] updated to cangulo-actions/semver@clean (#169)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@clean (#169)

# 26.0.0 test semver pr3 commit (#166)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-summary
## others
* ---------

# 25.0.1 fix: [cd.yml] updated to cangulo-actions/semver@fix-summary (#164)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-summary (#164)

# 25.0.0 test semver pr3 commit (#162)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-e2e
## others
* ---------

# 24.0.1 fix: [cd.yml] updated to cangulo-actions/semver@fix-e2e (#160)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-e2e (#160)

# 24.0.0 test semver pr3 commit (#158)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-e2e
## others
* ---------

# 23.0.1 fix: [cd.yml] updated to cangulo-actions/semver@fix-e2e (#156)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-e2e (#156)

# 23.0.0 test semver pr3 commit (#154)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-summary
## others
* ---------

# 22.0.2 fix: [cd.yml] updated to cangulo-actions/semver@fix-summary (#152)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-summary (#152)

# 22.0.1 fix: [cd.yml] updated to cangulo-actions/semver@fix-summary (#151)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-summary (#151)

# 22.0.0 test semver pr3 commit (#149)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@refactor-workflows
## others
* ---------

# 21.0.1 fix: [cd.yml] updated to cangulo-actions/semver@refactor-workflows (#147)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@refactor-workflows (#147)

# 21.0.0 test semver pr3 commit (#146)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@refactor-workflows
## others
* ---------

# 20.0.2 fix: [cd.yml] updated to cangulo-actions/semver@refactor-workflows (#145)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@refactor-workflows (#145)

# 20.0.1 fix: updated CD to use (#144)

## patches:
* fix: updated CD to use (#144)

# 20.0.0 test semver pr3 commit (#143)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-release-link
## others
* ---------

# 19.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-release-link (#142)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-release-link (#142)

# 19.0.0 test semver pr3 commit (#141)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-ci
## others
* ---------

# 18.0.1 fix: [cd.yml] updated to cangulo-actions/semver@fix-ci (#140)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-ci (#140)

# 18.0.0 test semver pr3 commit (#139)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-ci
## others
* ---------

# 17.0.1 fix: [cd.yml] updated to cangulo-actions/semver@fix-ci (#138)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@fix-ci (#138)

# 17.0.0 test semver pr3 commit (#137)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 16.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#136)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#136)

# 16.0.0 test semver pr3 commit (#135)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 15.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#134)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#134)

# 15.0.0 test semver pr3 commit (#133)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 14.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#132)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#132)

# 14.0.0 test semver pr3 commit (#131)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 13.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#130)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#130)

# 13.0.0 test semver pr1 commit (#129)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 12.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#128)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#128)

# 12.0.0 test semver pr1 commit (#127)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 11.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#126)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#126)

# 11.0.0 test semver pr1 commit (#125)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 10.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#124)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#124)

# 10.0.0 test semver pr1 commit (#123)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 9.0.17 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#122)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#122)

# 9.0.16 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#121)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#121)

# 9.0.15 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#120)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#120)

# 9.0.14 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#119)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#119)

# 9.0.13 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#118)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#118)

# 9.0.12 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#117)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#117)

# 9.0.11 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#116)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#116)

# 9.0.10 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#115)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#115)

# 9.0.9 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#114)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#114)

# 9.0.8 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#113)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#113)

# 9.0.7 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#112)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#112)

# 9.0.6 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#111)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#111)

# 9.0.5 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#110)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#110)

# 9.0.4 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#109)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#109)

# 9.0.3 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#108)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#108)

# 9.0.2 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#107)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#107)

# 9.0.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#106)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#106)

# 9.0.0 test semver pr1 commit (#103)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 8.3.8 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#101)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#101)

# 8.3.7 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#100)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#100)

# 8.3.6 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#99)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#99)

# 8.3.5 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#98)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#98)

# 8.3.4 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#97)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#97)

# 8.3.3 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#96)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#96)

# 8.3.2 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#95)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#95)

# 8.3.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#94)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#94)

# 8.3.0 test (#75)

## new features:
* feat: random commit that adds a feature
* feat: random commit that adds another feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 8.2.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#74)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#74)

# 8.2.0 test (#73)

## new features:
* feat: random commit that adds a feature
* feat: random commit that adds another feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 8.1.1 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#72)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#72)

# 8.1.0 test (#71)

## new features:
* feat: random commit that adds a new feature
## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests
## others
* ---------

# 8.0.4 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#70)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#70)

# 8.0.3 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#69)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#69)

# 8.0.2 fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#68)

## patches:
* fix: [cd.yml] updated to cangulo-actions/semver@add-e2e-tests (#68)

# 8.0.1 fix: [cd.yml] updated to target cangulo-actions/semver@main

## patches:
* fix: [cd.yml] updated to target cangulo-actions/semver@main

# 8.0.0 test (#52)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes something

# 7.0.7 fix: #123 random commit that fixes something (#51)

## patches:
* fix: #123 random commit that fixes something (#51)

# 7.0.6 fix: #123 random commit that fixes something

## patches:
* fix: #123 random commit that fixes something

# 7.0.5 fix: #123 random commit that fixes something

## patches:
* fix: #123 random commit that fixes something

# 7.0.4 fix: #123 random commit that fixes something

## patches:
* fix: #123 random commit that fixes something

# 7.0.3 fix: updated cd workflow 8

## patches:
* fix: updated cd workflow 8

# 7.0.2 fix: updated cd workflow 5

## patches:
* fix: updated cd workflow 5

# 7.0.1 fix: updated cd workflow 4

## patches:
* fix: updated cd workflow 4

# 7.0.0 test (#50)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes something

# 6.0.1 fix: enabled create-gh-release

## patches:
* fix: enabled create-gh-release

# 6.0.0 test (#49)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes something

# 5.0.0 test (#48)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes something

# 4.0.3 test (#47)

## patches:
* fix: #123 random commit that fixes something

# 4.0.2 test (#46)

## patches:
* fix: #123 random commit that fixes something

# 4.0.1 test (#45)

## patches:
* fix: #123 random commit that fixes something

# 4.0.0 test (#44)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes something

# 3.0.0 test (#43)

## breaking changes:
* break: #123 random commit that breaks something
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes something

# 2.0.14 test (#42)

## patches:
* fix: #123 random commit that fixes something

# 2.0.13 fix: #123 random commit that fixes something (#41)

## patches:
* fix: #123 random commit that fixes something (#41)

# 2.0.12 fix: #123 random commit that fixes something (#40)

## patches:
* fix: #123 random commit that fixes something (#40)

# 2.0.11 fix: #123 random commit that fixes something (#39)

## patches:
* fix: #123 random commit that fixes something (#39)

# 2.0.10 fix: #123 random commit that fixes something (#38)

## patches:
* fix: #123 random commit that fixes something (#38)

# 2.0.9 fix: #123 random commit that fixes something (#37)

## patches:
* fix: #123 random commit that fixes something (#37)

# 2.0.8 fix: #123 random commit that fixes something (#36)

## patches:
* fix: #123 random commit that fixes something (#36)

# 2.0.7 fix: #123 random commit that fixes something (#35)

## patches:
* fix: #123 random commit that fixes something (#35)

# 2.0.6 fix: #123 random commit that fixes something (#34)

## patches:
* fix: #123 random commit that fixes something (#34)

# 2.0.5 fix: #123 random commit that fixes something (#33)

## patches:
* fix: #123 random commit that fixes something (#33)

# 2.0.4 fix: #123 random commit that fixes something (#32)

## patches:
* fix: #123 random commit that fixes something (#32)

# 2.0.3 fix: #123 random commit that fixes something (#31)

## patches:
* fix: #123 random commit that fixes something (#31)

# 2.0.2 fix: updated cd workflow2

## patches:
* fix: updated cd workflow2

# 2.0.1 test (#29)

## patches:
* fix: #123 random commit that fixes something

# 2.0.0 test (#28)

## breaking changes:
* break: #123 random commit that fixes something
## new features:
* feat: #123 random commit that fixes something
## patches:
* fix: #123 random commit that fixes something

# 1.0.15 test (#27)

## patches:
* fix: #123 random commit that fixes something

# 1.0.14 fix: #123 random commit that fixes something (#26)

## patches:
* fix: #123 random commit that fixes something (#26)

# 1.0.13 fix: #123 random commit that fixes something (#25)

## patches:
* fix: #123 random commit that fixes something (#25)

# 1.0.12 fix: #123 random commit that fixes something (#24)

## patches:
* fix: #123 random commit that fixes something (#24)

# 1.0.11 fix: #123 random commit (#23)

## patches:
* fix: #123 random commit (#23)

# 1.0.10 fix: #123 random commit that fixes a bug (#22)

## patches:
* fix: #123 random commit that fixes a bug (#22)

# 1.0.9 fix: #123 random commit that fixes a bug (#21)

## patches:
* fix: #123 random commit that fixes a bug (#21)

# 1.0.8 fix: updated cd workflow2

## patches:
* fix: updated cd workflow2

# 1.0.7 fix: updated cd workflow2

## patches:
* fix: updated cd workflow2

# 1.0.6 fix: updated cd workflow

## patches:
* fix: updated cd workflow

# 1.0.5 fix: #123 random commit that fixes a bug (#20)

## patches:
* fix: #123 random commit that fixes a bug (#20)

# 1.0.4 fix: #123 random commit that fixes a bug (#19)

## patches:
* fix: #123 random commit that fixes a bug (#19)

# 1.0.3 test (#17)

## patches:
* fix: #123 random commit that fixes a bug

# 1.0.2 fix: #123 random commit that fixes a bug (#16)

## patches:
* fix: #123 random commit that fixes a bug (#16)

# 1.0.1 fix: #123 random commit that fixes a bug (#15)

## patches:
* fix: #123 random commit that fixes a bug (#15)

# 1.0.0 test (#14)

## breaking changes:
* break: #123 random commit that introduces a breaking change
## new features:
* feat: #123 random commit that adds a feature
## patches:
* fix: #123 random commit that fixes a bug
## others
* improved script push commit
* moved script to .vscode

# 0.0.5 test (#13)

## patches:
* fix: #123 INIT 2023-11-24 00:37:24

# 0.0.4 PR title  (#12)

## patches:
* fix: #123 INIT 2023-11-24 00:37:24
* asdasda sd asd asd
* break: #123 INIT 2023-11-24 00:37:24

# 0.0.3 PR title  (#12)

## patches:
* fix: #123 INIT 2023-11-24 00:37:24
* asdasda sd asd asd
* break: #123 INIT 2023-11-24 00:37:24

# 0.0.2 PR title  (#12)

## patches:
* fix: #123 INIT 2023-11-24 00:37:24
* asdasda sd asd asd
* break: #123 INIT 2023-11-24 00:37:24

# 0.0.1 PR title  (#12)

## patches:
* fix: #123 INIT 2023-11-24 00:37:24
* asdasda sd asd asd
* break: #123 INIT 2023-11-24 00:37:24

